...
class CreateIosPush {
  ...
  async provideOrGenerate(ctx) {
    ...
    if (userProvided) {
      if (!ctx.hasAppleCtx()) {
        (0, _log().default)(_chalk().default.yellow("WARNING! Unable to validate Push Key due to insufficient Apple Credentials. Please double check that you're uploading valid files for your app otherwise you may encounter strange errors!"));
        return userProvided;
      }
...
async function getValidPushKeys(iosCredentials, appleCtx) {
  const pushKeys = iosCredentials.userCredentials.filter(cred => cred.type === 'push-key');

  if (!appleCtx) {
    (0, _log().default)(_chalk().default.yellow(`Unable to determine validity of Push Keys.`));
    return pushKeys;
  }

  const pushKeyManager = new (_appleApi().PushKeyManager)(appleCtx);
...
async function validatePushKey(appleContext, pushKey) {
  const spinner = (0, _ora().default)(`Checking validity of push key on Apple Developer Portal...`).start();
  const pushKeyManager = new (_appleApi().PushKeyManager)(appleContext);
  const pushInfoFromApple = await pushKeyManager.list();
