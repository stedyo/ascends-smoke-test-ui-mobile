...
async function upgradeAsync(requestedSdkVersion, options) {
  ...
  const platforms = exp.platforms || []; // Check if we can, and probably should, upgrade the (ios) simulator

  if (_xdl().Simulator.isPlatformSupported() && platforms.includes('ios')) {
    let answer = await (0, _prompt().default)({
      type: 'confirm',
  ...
  if (_xdl().Android.isPlatformSupported() && platforms.includes('android')) {
    let answer = await (0, _prompt().default)({
      type: 'confirm',
      name: 'upgradeAndroid',
    ...
    if (answer.upgradeAndroid) {
      let result = await _xdl().Android.upgradeExpoAsync();
